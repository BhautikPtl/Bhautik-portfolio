      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {semesters.map((sem, index) => (
            ...
            >
              <div className={`w-16 h-16 bg-${sem.color}-500/10 text-${sem.color}-400 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 border border-${sem.color}-500/20`}>
                {sem.icon}
              </div>
              ...
              <div className="flex items-end gap-2 text-slate-400 font-medium h-12">
                <span className={`text-4xl font-black text-${sem.color}-400 leading-none ${sem.isCurrent ? 'text-3xl mt-2' : ''}`}>{sem.sgpa}</span>
          ...
          <div className="flex flex-wrap gap-4">
            {[
              ...
              <motion.div
                ...
                className="px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-slate-300 font-medium hover:bg-white/10 hover:border-white/20 transition-all cursor-default"
              >
                {course}
              </motion.div>
            ))}
